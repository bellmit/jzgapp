...
public class HttpServiceCar
{
	...
	public static void getMakeList(final Context context, final Handler mHandler,
			...
			, final String newOrOldId)
	{
		if (!checkNetwork(mHandler))
			return;
		StringRequest jsonObjRequest;
		String url = HttpServiceHelper.BASE_URL_COMMON_new
		...
		jsonObjRequest = new StringRequest(Request.Method.POST, url,
				new Response.Listener<String>()
				{
					...
					public void onResponse(String response)
					{
						try
						{
							Log.i("serviceValuation", "返回品牌"+response);
							JsonObjectImplCar jsonObjectImpl = new JsonObjectImplCar();
							if(JsonObjectImplCar.isSuccess(context, response)){
				...
				})
		{
			...
			protected Map<String, String> getParams() throws AuthFailureError
			{
				...
				params.put("InSale", newOrOldId);
				Map<String, Object> md = new HashMap<String, Object>();
				md.put("op", "GetMake");
				md.put("tokenid", tokenid);
				md.put("InSale", newOrOldId);
	...
	public static void getModelList(final Context context, final Handler mHandler,
									...
									final String tokenid, final String InSale)
	{
		if (!checkNetwork(mHandler))
			return;
		StringRequest jsonObjRequest;
		...
		jsonObjRequest = new StringRequest(Request.Method.POST, url,
				new Response.Listener<String>()
				{
					...
					public void onResponse(String response)
					{
						try
						{
							Log.i("serviceValuation", "返回车系"+response);
							JsonObjectImplCar jsonObjectImpl = new JsonObjectImplCar();
							if(JsonObjectImplCar.isSuccess(context, response)){
		...
		jsonObjRequest = new StringRequest(Request.Method.POST, url,
				new Response.Listener<String>()
				{
					...
					public void onResponse(String response)
					{
						try
						{
							Log.i("serviceValuation", "返回车型"+response);
							JsonObjectImplCar jsonObjectImpl = new JsonObjectImplCar();
							if(JsonObjectImplCar.isSuccess(context, response)){
